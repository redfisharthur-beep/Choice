...
function render(){
  ...
  $('#voteBtn').disabled=!selectedVote||data.options.length<2||data.phase!=='voting';
  $('#closeVoteBtn').classList.toggle('hidden',!isHost||data.phase!=='voting');
...
async function createRoom(){
  ...
  try{const r=await fetch(api('/api/rooms'),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({password,state:roomState()})});const j=await r.json();if(!r.ok)throw new Error(j.error||'建立失敗');data.roomCode=j.code;data.hostToken=j.hostToken||'';isHost=true;save();$('#openRoomDialog').close();connectRoom();render();showStep('setup',true);toast(`房間 ${j.code} 已建立`)}catch(e){toast(e.message||'無法建立房間')}
}
...
function attemptJoin(code,password=''){code=String(code||'').trim().toUpperCase();if(!/^[A-Z0-9]{6}$/.test(code))return toast('請輸入 6 位房號');data.roomCode=code;data.roomPassword=password;data.hostToken='';isHost=false;pendingJoinCode=code;save();render();connectRoom()}
function leaveRoom(){try{roomSocket?.close()}catch{}roomSocket=null;roomMembers=[];data.roomCode=null;data.roomPassword='';data.hostToken='';data.myVote=null;isHost=false;selectedVote=null;save();history.replaceState({},'',location.pathname);render();showStep('setup',true)}
...
$('#createRoomBtn').onclick=createRoom;$('#quickJoinBtn').onclick=()=>attemptJoin($('#quickJoinCode').value);$('#quickJoinCode').onkeydown=e=>{if(e.key==='Enter')attemptJoin(e.target.value)};
...
$('#goVoteBtn').onclick=()=>{if(data.options.length<2)return toast('至少要有 2 個項目');send({type:'phase',phase:'voting'})};
$('#goDrawBtn').onclick=()=>{if(data.options.length<2)return toast('至少要有 2 個項目');send({type:'phase',phase:'draw'})};
$('#voteOptions').onclick=e=>{const b=e.target.closest('[data-vote]');if(!b||data.phase!=='voting')return;selectedVote=b.dataset.vote;render()};
$('#voteBtn').onclick=()=>{if(!selectedVote||data.phase!=='voting')return;send({type:'vote',optionId:selectedVote})};
$('#closeVoteBtn').onclick=()=>send({type:'phase',phase:'closed'});
$('#analysisDrawBtn').onclick=()=>send({type:'phase',phase:'draw'});
...
function announce(result){if(!isHost)return;send({type:'announce',result});showAnnouncement(result)}function showAnnouncement(result){$('#announceResult').textContent=result;$('#announceDialog').showModal()}
