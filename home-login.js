(()=>{
  ...
  function initMorandiDeadlinePicker(){
    const native=document.querySelector('#voteDeadlineInput');
    if(!native||document.querySelector('#morandiDeadlineBtn'))return;
    ...
    style.textContent=`
      ...
      .morandi-deadline-btn{width:100%;min-height:58px;border:1px solid rgba(103,126,119,.12);border-radius:16px;background:#f3f2ec;color:#526b73;font:inherit;font-size:18px;padding:0 18px;display:flex;align-items:center;justify-content:space-between;gap:12px;cursor:pointer;box-shadow:0 5px 14px rgba(83,102,96,.05)}
      .morandi-deadline-btn::after{content:'▾';display:grid;place-items:center;width:34px;height:34px;border-radius:12px;background:#cadbd5;color:#4f7168;font-size:16px;flex:0 0 auto}
      .morandi-deadline-btn.has-value{background:#edf2ef;color:#3f665f}
      ...
      .morandi-picker-head{display:grid;grid-template-columns:48px 1fr 48px;align-items:center;gap:10px;margin-bottom:15px}
      .morandi-picker-head button{border:0;width:44px;height:44px;border-radius:14px;background:#d9e3dc;color:#4f6e66;font-size:24px;cursor:pointer}
      .morandi-picker-month{text-align:center;font-size:22px;letter-spacing:.05em}
      .morandi-weekdays,.morandi-days{display:grid;grid-template-columns:repeat(7,1fr);gap:7px}
      .morandi-weekdays{margin-bottom:7px}.morandi-weekdays span{text-align:center;font-size:13px;color:#879590;padding:6px 0}
      .morandi-day{aspect-ratio:1/1;border:0;border-radius:14px;background:#ebece6;color:#516871;font-size:16px;cursor:pointer;transition:transform .12s ease,background .12s ease}
      .morandi-day:hover:not(:disabled){transform:translateY(-1px);background:#dbe7e1}.morandi-day.other{opacity:.38}.morandi-day.today{box-shadow:inset 0 0 0 1.5px #9dbdb4}.morandi-day.selected{background:#b9d7cf;color:#315f56;box-shadow:0 5px 12px rgba(105,147,135,.18)}.morandi-day:disabled{opacity:.22;cursor:not-allowed}
      .morandi-time-card{margin-top:18px;padding:16px;border-radius:20px;background:#e6e6dd}
      .morandi-time-title{text-align:center;font-size:15px;color:#788b88;margin-bottom:12px}
      .morandi-period{display:flex;justify-content:center;gap:8px;margin-bottom:12px}.morandi-period button{border:0;border-radius:999px;padding:9px 20px;background:#dddccf;color:#65767a;font-size:15px;cursor:pointer}.morandi-period button.active{background:#c9dae6;color:#466777}
      .morandi-time-row{display:grid;grid-template-columns:44px minmax(66px,1fr) 44px 18px 44px minmax(66px,1fr) 44px;gap:7px;align-items:center}
      .morandi-time-row button{border:0;height:42px;border-radius:999px;background:#d2dfda;color:#4b6e65;font-size:22px;cursor:pointer}.morandi-time-value{height:44px;border-radius:999px;background:#d7e2ec;display:grid;place-items:center;color:#486a78;font-size:19px}.morandi-colon{text-align:center;font-size:20px;color:#778a87}
      ...
      @keyframes morandiFade{from{opacity:0}to{opacity:1}}
      @media(max-width:600px){.morandi-picker{padding:17px;border-radius:23px}.morandi-picker-month{font-size:20px}.morandi-day{border-radius:12px;font-size:15px}.morandi-time-row{grid-template-columns:40px 1fr 40px 14px 40px 1fr 40px;gap:5px}.morandi-deadline-btn{font-size:16px}}
    `;
    document.head.appendChild(style);

    const display=document.createElement('button');
    display.id='morandiDeadlineBtn';
    display.type='button';
    display.className='morandi-deadline-btn';
    display.textContent='選擇日期與時間';
    native.insertAdjacentElement('afterend',display);
    ...
    const formatValue=value=>{
      ...
      if(Number.isNaN(d.getTime()))return '選擇日期與時間';
      return `${d.getFullYear()} / ${pad(d.getMonth()+1)} / ${pad(d.getDate())}　${d.getHours()<12?'上午':'下午'} ${pad(d.getHours()%12||12)}:${pad(d.getMinutes())}`;
    };
    const refreshDisplay=()=>{
      display.textContent=formatValue(native.value);
      display.classList.toggle('has-value',!!native.value);
      const disabled=typeof data!=='undefined'&&(!isHost||data.phase!=='setup');
      display.disabled=!!disabled;
      display.style.opacity=disabled?'.55':'1';
    };

    function openPicker(){
      if(display.disabled)return;
      const now=new Date();
      let selected=native.value?new Date(native.value):new Date(now.getTime()+60*60*1000);
      if(Number.isNaN(selected.getTime()))selected=new Date(now.getTime()+60*60*1000);
      ...
      backdrop.className='morandi-picker-backdrop';
      backdrop.innerHTML=`<div class="morandi-picker" role="dialog" aria-modal="true" aria-label="選擇投票截止時間">
        <div class="morandi-picker-head"><button type="button" data-prev aria-label="上個月">‹</button><div class="morandi-picker-month"></div><button type="button" data-next aria-label="下個月">›</button></div>
        <div class="morandi-weekdays"><span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span></div>
        <div class="morandi-days"></div>
        <div class="morandi-time-card"><div class="morandi-time-title">選擇時間</div><div class="morandi-period"><button type="button" data-period="am">上午</button><button type="button" data-period="pm">下午</button></div>
          <div class="morandi-time-row"><button type="button" data-hminus>−</button><div class="morandi-time-value" data-hour></div><button type="button" data-hplus>＋</button><div class="morandi-colon">:</div><button type="button" data-mminus>−</button><div class="morandi-time-value" data-minute></div><button type="button" data-mplus>＋</button></div>
        </div>
        <div class="morandi-picker-actions"><button type="button" class="morandi-clear" data-clear>清除</button><button type="button" class="morandi-confirm" data-confirm>確定</button></div>
      </div>`;
      document.body.appendChild(backdrop);
      const monthLabel=backdrop.querySelector('.morandi-picker-month'),days=backdrop.querySelector('.morandi-days');
      const hourEl=backdrop.querySelector('[data-hour]'),minuteEl=backdrop.querySelector('[data-minute]');

      const renderTime=()=>{
        period=hour>=12?'pm':'am';
        hourEl.textContent=pad(hour%12||12);minuteEl.textContent=pad(minute);
        backdrop.querySelectorAll('[data-period]').forEach(b=>b.classList.toggle('active',b.dataset.period===period));
      };
      ...
      const renderDays=()=>{
        ...
        for(let i=0;i<42;i++){
          ...
          const b=document.createElement('button');b.type='button';b.className='morandi-day';b.textContent=String(d.getDate());
          if(d.getMonth()!==view.getMonth())b.classList.add('other');
          if(sameDate(d,new Date()))b.classList.add('today');
          if(sameDate(d,selected))b.classList.add('selected');
          const dayOnly=new Date(d);dayOnly.setHours(0,0,0,0);if(dayOnly<today)b.disabled=true;
          b.onclick=()=>{selected=new Date(d.getFullYear(),d.getMonth(),d.getDate(),hour,minute);view=new Date(d.getFullYear(),d.getMonth(),1);renderDays()};
          days.appendChild(b);
        }
      };
      const syncSelectedTime=()=>{selected.setHours(hour,minute,0,0);renderTime()};
      backdrop.querySelector('[data-prev]').onclick=()=>{view=new Date(view.getFullYear(),view.getMonth()-1,1);renderDays()};
      backdrop.querySelector('[data-next]').onclick=()=>{view=new Date(view.getFullYear(),view.getMonth()+1,1);renderDays()};
      backdrop.querySelectorAll('[data-period]').forEach(b=>b.onclick=()=>{if(b.dataset.period==='am'&&hour>=12)hour-=12;if(b.dataset.period==='pm'&&hour<12)hour+=12;syncSelectedTime()});
      backdrop.querySelector('[data-hminus]').onclick=()=>{hour=(hour+23)%24;syncSelectedTime()};
      backdrop.querySelector('[data-hplus]').onclick=()=>{hour=(hour+1)%24;syncSelectedTime()};
      backdrop.querySelector('[data-mminus]').onclick=()=>{minute=(minute+55)%60;syncSelectedTime()};
      backdrop.querySelector('[data-mplus]').onclick=()=>{minute=(minute+5)%60;syncSelectedTime()};
      backdrop.querySelector('[data-clear]').onclick=()=>{native.value='';native.dispatchEvent(new Event('change',{bubbles:true}));refreshDisplay();backdrop.remove()};
      backdrop.querySelector('[data-confirm]').onclick=()=>{
        selected.setHours(hour,minute,0,0);
        if(selected.getTime()<=Date.now()){showToast('截止時間要晚於現在');return}
        native.value=toLocalValue(selected);native.dispatchEvent(new Event('change',{bubbles:true}));refreshDisplay();backdrop.remove();
      };
      backdrop.addEventListener('click',e=>{if(e.target===backdrop)backdrop.remove()});
      renderDays();renderTime();
    }

    display.addEventListener('click',openPicker);
    native.addEventListener('change',refreshDisplay);
    setInterval(refreshDisplay,1000);
    refreshDisplay();
  }

  initMorandiDeadlinePicker();
